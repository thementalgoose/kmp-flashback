...
internal class RemoteNotificationsUnsubscribeUseCaseTest {
    ...
    @Test
    fun `invoke unsubscribes from topic via firebase service`() = runTest {
    ...
    @Test
    fun `invoke removes topic from repository`() = runTest {
        val topic = "test-topic"
    ...
    @Test
    fun `invoke removes only specified topic from multiple topics`() = runTest {
    ...
    @Test
    fun `invoke handles removing non-existent topic`() = runTest {
